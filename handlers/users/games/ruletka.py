...
async def ruletla_handler(message: Message):
    ...
    if flood and flood2:
        ...
        if len(arg) == 0:
            return await message.reply('❌ Ошибка. Используйте: <code>Рулетка (<i>ставка</i>)</code>')
        ...
        return await message.reply(f'Вы начали играть в игру «Русская рулетка» \n'
...
async def rulet_push_handler(message: Message):
    ...
    if flood and flood2:
        ...
        if user.state_ruletka is None:
            return await message.reply("❌ Начни игру\n"
...
async def rulet_call(callback_query: CallbackQuery):
    ...
    if user_id != callback_query.from_user.id:
        return await callback_query.answer("❌ Не трожь не твое")
    if user.state_ruletka is None:
        ...
        if user.state_ruletka is not None:
            ...
            if state2 >= 1:
                ...
                return await callback_query.message.edit_text(f'{user.link}, Вы не сделали ни одного выстрела 😞\n'
                                                              ...
                                                              disable_web_page_preview=True)
        if summ <= 0:
            return await callback_query.message.edit_text('❌ Ошибка. Ставка меньше или равна нулю')
        if user.balance < summ:
            return await callback_query.message.edit_text('❌ Ошибка. Недостаточно денег на руках для ставки! 💸',
                                                          reply_markup=show_balance_kb.as_markup())
        user.edit('state_ruletka', f'{0},{summ}')
        return await callback_query.message.edit_text(f'Вы начали играть в игру «Русская рулетка» \n'
                                                      '🔫 Осталось выстрелов: 6 [ ×  ×  ×  ×  ×  × ]\n'
                                                      '❓ Введите «Выстрел» для продолжения, либо «Ррулетка» чтобы забрать приз 👍🏼\n',
