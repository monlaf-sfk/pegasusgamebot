...
async def basketball_handler(message: Message):
    ...
    if flood and flood2:
        ...
        if len(arg) == 0:
            return await message.reply(f'{user.link}, для игры в баскетбол введите «Баскетбол [ставка]» 👍🏼 \n'
        ...
        if basketball.value in values[3]:
            ...
            return await message.reply(f'🏀️ Вы умножили свою ставку на (x1.35) и получили +{to_str(summ)} на баланс!',
                                       reply_markup=play_basketball_kb.as_markup())
        elif basketball.value == 5:
            summ = int(summ * 2.5)
            user.edit('balance', user.balance + summ - ssumm)
            await asyncio.sleep(3)
            return await message.reply(f'🏀️ Вы умножили свою ставку на (x2.5) и получили +{to_str(summ)} на баланс!',
                                       reply_markup=play_basketball_kb.as_markup())
        else:
